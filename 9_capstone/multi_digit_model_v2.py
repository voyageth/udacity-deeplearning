...
class MultiDigitModel(object):
    ...
    def inference(self, data, is_training=True):
        if self.use_LCN:
            data = LecunLCN(data, data.get_shape())
        stride = 2
        conv = None
    ...
    def run(self):
        ...
        with self.graph.as_default():
            with tf.variable_scope("global_scope") as scope:
                ...
                train_prediction_logits = self.inference(tf_train_dataset, False)
                train_length_prediction = tf.nn.softmax(train_prediction_logits[0])
                train_digits_0_prediction = tf.nn.softmax(train_prediction_logits[1])
                train_digits_1_prediction = tf.nn.softmax(train_prediction_logits[2])
                train_digits_2_prediction = tf.nn.softmax(train_prediction_logits[3])
                train_digits_3_prediction = tf.nn.softmax(train_prediction_logits[4])
                train_digits_4_prediction = tf.nn.softmax(train_prediction_logits[5])
                ...
                for step in range(self.num_steps):
                    ...
                    if step % 500 == 0:
                        ...
                        tf.scalar_summary('train_length_accuracy', accuracy_result)
                        for k in range(0, self.digit_count):
                            ...
                            tf.scalar_summary('train_digit_accuracy', accuracy_result)
                        ...
                        tf.scalar_summary('train_accuracy', accuracy_result)
                        ...
                        tf.scalar_summary('validation_accuracy', accuracy_result)
                ...
                print("finish : {}".format(time.strftime("%Y-%m-%dT%H:%M:%S%z")))
                tf.scalar_summary('test_accuracy', accuracy_result)
